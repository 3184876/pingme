...
func traceroute() {
	var m = flag.Int("m", tr.DEFAULT_MAX_HOPS, `Set the max time-to-live (max number of hops) used in outgoing probe packets (default is 64)`)
	var f = flag.Int("f", tr.DEFAULT_FIRST_HOP, `Set the first used time-to-live, e.g. the first hop (default is 1)`)
	...
	c := make(chan tr.TracerouteHop, 0)
	go func() {
		for {
			hop, ok := <-c
			if !ok {
	...
	_, err = tr.Traceroute(host, &options, c)
	if err != nil {
